...
def do_init():
    ...
    global token_addr
    if token_addr == "":
        token_addr = deploy_contract("H:/XWC/xwc_nft/erc721.glua.gpc")
...
def mint(account,tokenId):
    res = invoke_contract(account,token_addr,"mint","%s,%s"%(account,tokenId))
    return res["result"]["exec_succeed"]

def safeMint(account,tokenId):
    res = invoke_contract(account, token_addr, "safeMint", "%s,%s" % (account, tokenId))
    print(res)
    return res["result"]["exec_succeed"]
...
class ERC721Test(unittest.TestCase):
    ...
    def test_mint(self):
        invoke_contract("test",token_addr,"mint","test,aaasssa")
    ...
    def test_mint2(self):
        invoke_contract("test", token_addr, "mint", "test,aaasssab")
